...
int main()
{
	string fileName = "./img./blackbox_2.jpg";
	...
	cv::findContours(star_result, contours, hierarchy, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);
	
	// Draw contours on the original image
	cv::Mat contourImage = org_img.clone();
	cv::drawContours(contourImage, contours, -1, cv::Scalar(0, 255, 0), 2);
	for (int i = 0; i < contours.size(); ++i) {
		double area = cv::contourArea(contours[i]);
		if (area > 500) {
			cv::fillPoly(contourImage, contours, cv::Scalar(255, 255, 255), 8, 0);
		}
	...
	// Create output labels and centers matrices
	Mat labels, centers;
	...
	// Apply erosion
	cv::Mat erodedImage;
	cv::erode(dilatedEdges, erodedImage, kernel);
	imshow(" erodedImage Image", erodedImage);
	cv::dilate(erodedImage, dilatedEdges, cv::Mat());
	imshow(" dilatedEdges Image", dilatedEdges);
	/*
